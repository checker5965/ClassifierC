...
int main(void)
{
  ...
  train = NULL;
  test = NULL;
  char* filename = "bikes.csv";
  train = listify(filename, 4, train);
  n = splitdata(&train, &test, 4);
  //display(train, 5);
  printf("%d\n", n);
  ...
  for(i = 0; i < 3; ++i)
  {
    ...
    mean_squared_error = MSE(test, a, b, i, 3);
    printf("MSE = %lg\n\n", mean_squared_error);
  }
